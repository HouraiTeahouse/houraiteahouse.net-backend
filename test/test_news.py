...
class NewsTest(HouraiTeahouseTestCase):
    ...
    def test_tag_succeeds_on_empty_tag(self):
        db.session.add(models.NewsTag('test'))
        response = self.client.get('/news/tag/test?language=en_US')
        self.assert200(response)

    def test_tag_fails_on_nonexistent_tag(self):
        response = self.client.get('/news/tag/test?language=en_US')
    ...
    def test_translate_requires_authentication(self):
        response = self.post('/news/1/translate')
        self.assert401(response)
